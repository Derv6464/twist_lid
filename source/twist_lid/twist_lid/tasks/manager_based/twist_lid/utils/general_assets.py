# from https://github.com/LightwheelAI/leisaac/blob/main/source/leisaac/leisaac/utils/general_assets.py
...
def parse_usd_and_create_subassets(usd_path, env_cfg, specific_name_list=None, exclude_name_list=None, postions=None):
    ...
    for prim in prims:
        if is_rigidbody(prim) and match_specific_name(prim.GetPath().pathString, specific_name_list, exclude_name_list):
            ...
            if postions is not None and name in postions:
                pos = postions[name]
            if name not in create_attr_record:
                create_attr_record[name] = 0
            else:
